...
function validatePostForm(titleInput, titleError, categoryInput, categoryError, contentInput, contentError) {
    let isValid = true;
...
export async function initEditPostForm(postId) {
    try {
        ...
        if (submitBtn)
            submitBtn.textContent = 'Зберегти пост';
        showView('Form');
    }
    catch (error) {
...
export async function handleCreatePostSubmit(formElement) {
    ...
    const isFormValid = validatePostForm(titleInput, titleError, categoryInput, categoryError, contentInput, contentError);
    if (!isFormValid) {
